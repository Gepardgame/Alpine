...
public class ApiKey implements Serializable, Principal {
    ...
    public static final char API_KEY_SEPERATOR = '_';
    public static int FULL_KEY_LENGTH = prefix.length() + PUBLIC_ID_LENGTH + API_KEY_LENGTH + 1;
     ...
     * @param isLegacy If the key is not generated by the new system
     * @return
     */
    public static String getOnlyKey(String key, boolean isLegacy) {
        if (isLegacy) {
            var startKey = prefix.length() + PUBLIC_ID_LENGTH;
            return key.substring(startKey);
        }

        String[] parts = key.split(Character.toString(API_KEY_SEPERATOR));
     ...
     */
    public static byte[] getOnlyKeyAsBytes(String key, boolean isLegacy) {
        return getOnlyKey(key, isLegacy).getBytes(StandardCharsets.UTF_8);
    }
     ...
     * @return Public ID
     */
    public static String getPublicId(String key, boolean isLegacy) {
        if (isLegacy) {
